  ...
  try {
    ...
    const [ticketStats] = await pool.query(`
      SELECT 
        ...
        SUM(CASE WHEN status IN ('in_progress', 'in_review') THEN 1 ELSE 0 END) as in_progress,
    ...
    const [userStats] = await pool.query(`
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN role IN ('moderator', 'staff') THEN id END) as moderator_count,
  ...
  try {
    const [staffStats] = await pool.query(`
      SELECT 
        ...
        SUM(CASE WHEN t.status IN ('in_progress', 'in_review') THEN 1 ELSE 0 END) as active_tickets,
        ROUND(AVG(
          CASE 
      ...
      WHERE u.role IN ('admin', 'moderator', 'staff')
